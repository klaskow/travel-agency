...
const parseTrips = (trips, setStates) => {
  ...
  for (let trip of trips) {
    ...
    if (typeof newState.countries[trip.country.code] == 'undefined') {
      const countryDetails =
        countries.filter(item => item.alpha3Code == trip.country.code)[0] || {};
      newState.countries[trip.country.code] = JSON.parse(
  ...
  for (let countryCode in newState.countries) {
    ...
    if (typeof newState.regions[country.region] == 'undefined') {
      ...
      newState.regions[country.region].subregions.indexOf(country.subregion) ==
      -1
    ) {
      ...
      newState.regions[country.region].countries.indexOf(country.alpha3Code) ==
      -1
    ) {
      newState.regions[country.region].countries.push(country.alpha3Code);
    }
  ...
  for (let option of pricing) {
    if (typeof option.defaultValue != 'undefined') {
      ...
      typeof option.limits != 'undefined' &&
      typeof option.limits.min != 'undefined'
    ) {
